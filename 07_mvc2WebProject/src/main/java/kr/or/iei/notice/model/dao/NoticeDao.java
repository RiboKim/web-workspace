...
public class NoticeDao {
	...
	public int selectNoticeCount(Connection conn) {
		...
		ResultSet rset = null;
		int totalCount=0;
		String query = "select count(*) as cnt from notice"; //이럴때 쓰라고 쿼리에 별칭을 쓰는거다
		try {
			pstmt = conn.prepareStatement(query);
			rset = pstmt.executeQuery();
			if(rset.next()) {
	...
	public int insertNoticeComment(Connection conn, NoticeComment nc) {
		...
		try {
			...
			System.out.println(nc.getNcRef());
			pstmt.setString(4, (nc.getNcRef() == 0)?null:String.valueOf(nc.getNcRef())); //삼항연산자
			/*
			if(nc.getNcRef() == 0) {
